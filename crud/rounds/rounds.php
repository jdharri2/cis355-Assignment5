<?php
	...
	class Rounds{ //implements Table {
		...
		<body> <h1> <a href="../persons/index.php">People</a> <a href="../courses/index.php">Courses</a> <a href="../rounds/index.php">Rounds</a></h1>
    		<div class="container">
            	<div class="row">
                	<h3>Rounds Table</h3>
            	</div>
	...
	public function displayUpdate(){
			...
			// if $_post is empty, then pre-fill data
				if ( !empty($_POST)) {
					...
					$valid = true;
					if (empty($course_id)) {
						$cIdError = 'Please enter Course ID';
						$valid = false;
					}
					 
					if (empty($person_id)) {
						$person_idError = 'Please enter Person ID';
						$valid = false;
					}
					if (empty($date)) {
				...
				<div class="container">
							...
							<div class="span10 offset1">
								...
								<form class="form-horizontal" action="update.php?id=<?php echo $id?>" method="post">
								  <div class="control-group <?php echo !empty($course_idError)?'error':'';?>">
									<label class="control-label">Course ID</label>
									<div class="controls">
										<input name="course_id" type="text"  placeholder="Course ID" value="<?php echo !empty($course_id)?$course_id:'';?>">
										<?php if (!empty($course_idError)): ?>
											<span class="help-inline"><?php echo $course_idError;?></span>
										<?php endif; ?>
									</div>
								  </div>
								  <div class="control-group <?php echo !empty($person_idError)?'error':'';?>">
									<label class="control-label">Person ID</label>
									<div class="controls">
										<input name="person_id" type="text"  placeholder="Person ID" value="<?php echo !empty($person_id)?$person_id:'';?>">
										<?php if (!empty($person_idError)): ?>
											<span class="help-inline"><?php echo $person_idError;?></span>
										<?php endif;?>
									</div>
								  </div>
								  <div class="control-group <?php echo !empty($dateError)?'error':'';?>">
